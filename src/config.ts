...
const schema = z.object({
  ...
  HERMES_AUTH_PROVIDER: z.string().optional().default(""),
  HERMES_LOGIN_PATH: z.string().default("/api/v1/auth/login"),
  HERMES_HEALTH_PATH: z.string().default("/health"),
  ...
  STT_PROVIDER: z.enum(["mock", "openai", "hermes", "elevenlabs"]).default("mock"),
  OPENAI_STT_MODEL: z.string().default("gpt-4o-mini-transcribe"),
  HERMES_STT_PATH: z.string().default("/api/v1/audio/transcriptions"),
  HERMES_STT_MODEL: z.string().default("local"),
  TTS_PROVIDER: z.enum(["none", "openai", "elevenlabs"]).default("openai"),
  ...
  ELEVENLABS_TTS_OUTPUT_FORMAT: z.string().default("pcm_16000"),
  ELEVENLABS_TIMEOUT_MS: z.coerce.number().int().positive().default(30000),
  VOICE_SESSION_MAX_SECONDS: z.coerce.number().int().positive().default(60),
  VOICE_AUDIO_SAMPLE_RATE: z.coerce.number().int().positive().default(16000),
  VOICE_AUDIO_CHUNK_MS: z.coerce.number().int().positive().default(40),
  VOICE_ASSISTANT_LANGUAGE: z.string().default("auto"),
  STORE_TRANSCRIPTS: z.preprocess(envBoolean, z.boolean()).default(true),
  STORE_VOICE_EVENTS: z.preprocess(envBoolean, z.boolean()).default(false),
  HERMES_TASK_POLL_ENABLED: z.preprocess(envBoolean, z.boolean()).default(true),
  HERMES_TASK_POLL_INTERVAL_MS: z.coerce.number().int().positive().default(5000),
  HERMES_WEBHOOK_SECRET: z.string().optional().default(""),
